...
public class GroupTaskService : IGroupTaskService
{
    ...
    public async Task<GroupTaskListResponseDto> GetAsync(int currentUserId, GroupTaskQueryDto query)
    {
        ...
        var pageSize = Math.Clamp(query.PageSize <= 0 ? 50 : query.PageSize, 1, 50);
        ...
        if (query.AssignedToUserId.HasValue)
            dbQuery = dbQuery.Where(t => t.AssignedToUserId == query.AssignedToUserId.Value);
    ...
    public async Task<(GroupTaskResponseDto? Task, string? Error)> UpdateAsync(int currentUserId, int id, UpdateGroupTaskDto dto)
    {
        ...
        var isAdminOrCoAdmin = await IsAdminOrCoAdminAsync(currentUserId, task.GroupId);
        var isCreator = task.CreatedByUserId == currentUserId;
        var isAssigneeOnly = task.AssignedToUserId == currentUserId && !isCreator && !isAdminOrCoAdmin;

        if (isAssigneeOnly && (
            dto.Title != null ||
            dto.Description != null ||
            dto.AssignedToUserId.HasValue ||
            dto.DueAtUtc.HasValue ||
            !string.IsNullOrWhiteSpace(dto.Priority)))
        {
            return (null, "Người được giao chỉ được cập nhật trạng thái task.");
        }
        ...
        if (dto.AssignedToUserId.HasValue)
        {
            ...
            if (dto.AssignedToUserId.Value != currentUserId && !isAdminOrCoAdmin)
                return (null, "Member thường chỉ được assign task cho chính mình.");
    ...
    private async Task<(bool Success, string? Error)> ValidateCreateAsync(int currentUserId, CreateGroupTaskDto dto)
    {
        ...
        if (dto.AssignedToUserId.HasValue &&
            dto.AssignedToUserId.Value != currentUserId &&
            !await IsAdminOrCoAdminAsync(currentUserId, dto.GroupId))
        {
            return (false, "Member thường chỉ được tạo task cho chính mình hoặc để unassigned.");
        }
    ...
    {
        if (task.CreatedByUserId == userId)
            return task.Status != DoneStatus && task.Status != CancelledStatus;
