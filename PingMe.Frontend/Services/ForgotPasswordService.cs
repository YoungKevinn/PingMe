...
public class ForgotPasswordService : ApiService
{
    ...
    /// <summary>
    /// Yêu cầu gửi mật khẩu mới về email đã đăng ký. Mật khẩu user sẽ được reset theo mật khẩu gửi về.
    /// </summary>
    public async Task<(bool success, string? error)> SendNewPasswordAsync(string email)
    {
        var response = await PostAsync("auth/forgot-password/send-new-password", new { email });
