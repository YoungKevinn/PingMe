...
const pmFmt = {
    // Parse an ISO timestamp as UTC when it has no timezone designator
    // (server stores DateTime.UtcNow → MySQL may return it without 'Z').
    _parse(iso) {
        if (!iso) return null;
        let s = String(iso);
    ...
    // Exact timestamp: "HH:mm dd/MM/yyyy"
    datetime(iso) {
        const d = this._parse(iso);
        if (!d) return '';
